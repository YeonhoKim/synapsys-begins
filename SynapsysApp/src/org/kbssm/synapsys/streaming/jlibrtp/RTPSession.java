...
package org.kbssm.synapsys.streaming.jlibrtp;
...
public class RTPSession {
	 ...
	 /** SDES CNAME */
	 protected String cname = null;		// Session 생성시 추가
	  ...
	  * 
	  * @param buffers A buffer of bytes, should not bed padded and less than 65535(1500->65535) bytes on most networks.
	 ...
	 public long[][] sendData(byte[][] buffers, long[] csrcArray, boolean[] markers, long rtpTimestamp, long[] seqNumbers) {
		 ...
		 for(int i=0; i<buffers.length; i++) {
			 ...
			 if(buf.length > bufferSize) {
				 System.out.println("RTPSession.sendData() called with buffer exceeding 65535(1500->65535) bytes ("+buf.length+")");
			 }
	...
	private void generateCNAME() {
		...
		InetAddress address;
		if(this.mcSession) {
			address = this.rtpMCSock.getLocalAddress();

		} else {
			address = this.rtpSock.getLocalAddress();
		}
		
		if (address != null) {
			cname = "Android@" + address.getCanonicalHostName();
